...
def execution(tmp_path, request):
    tasks = (
        ...
        if request.param == "database" else LocalTaskStore(tmp_path)
    )
    sessions = (
        DatabaseSessionStore(f"sqlite:///{tmp_path}/tasks.db", create_schema=True)
        if request.param == "database" else LocalSessionStore(tmp_path)
    )
    registry = DaemonNodeRegistry(sessions, LocalDaemonStore(tmp_path), task_store=tasks)
    registry.register({
        "sandboxId": "node", "employeeId": "alice", "token": "node_token",
        "workspacePath": "/workspace/alice", "protocolVersion": 1,
        "supportedAgents": ["codex"],
        "capabilities": ["task-workspaces", "thread-workspaces"], "status": "ready",
    }, "ui_token")
    agents = LocalAgentStore(tmp_path)
    placements = LocalAgentPlacementStore(tmp_path)
    agent = agents.create_agent("alice", {"displayName": "Builder", "executorKind": "codex", "defaultRole": "implementer"})
    placements.create_placement(agent, "node")
    backend = ServerDaemonNodeBackend(registry, employee_agent_store=agents, agent_placement_store=placements)
    task = tasks.create_task({
        "title": "Deliver work", "assignedAgent": "codex", "assignedAgentId": agent["id"],
        "assigneeEmployeeId": "alice", "status": "assigned",
    })
    ctx = SimpleNamespace(
        task_store=tasks, session_store=sessions, registry=registry, backend=backend,
        agent_store=agents, agent_placement_store=placements,
        team_store=None, project_store=None, managed_node_store=None,
    )
    return ctx, task, agent


def scheduler(ctx):
    return TaskScheduler(task_store=ctx.task_store, registry=ctx.registry, backend=ctx.backend)
...
def test_disabled_agent_records_actionable_blocker(execution, source):
    ...
    if source == "manual":
        asyncio.run(start_task_on_ready_node(ctx, task, {"employeeId": "alice", "isAdmin": False}))
    else:
...
def test_retry_exhaustion_records_actionable_blocker(execution):
    ...
    blocked = record_dispatch_retry(
        ctx.task_store, task, code="capacity_exhausted", message="Node is full",
        max_failures=1, sample=lambda: 0.5,
    )
    assert blocked["status"] == "blocked"
...
def test_scheduler_waits_for_daemon_execution_and_does_not_redispatch(execution):
    ...
    ctx.registry.handle_event("node", {
        "type": "run.executing", "commandId": command["id"],
