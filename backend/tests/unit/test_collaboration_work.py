...
def test_required_failure_blocks_legacy_completion_but_optional_does_not():
    items = assignments()
    state = {
        "_relay_participant_failures": [{"assignmentId": "verify", "outcome": "failed"}]
    }
...
def test_optional_reviewer_findings_on_required_work_cannot_be_ignored():
    items = assignments()
    items[2]["required"] = False
    state = {WORK_RESULTS: {item["assignmentId"]: result() for item in items}}
    state = record_work_result(
        state,
        items[2],
        result(
            "continue", findings=[{"workItemId": "build", "note": "Fails acceptance"}]
        ),
    )
    assert completion_blockers(items, state, require_evidence=True)
...
def test_review_defects_return_to_owner_and_invalidate_downstream_evidence():
    ...
    reported = result(
        "continue", findings=[{"workItemId": "build", "note": "Empty input crashes"}]
    )
    ...
    assert state == before
    assert (
        repair_transition(items, 2, {**state, "_relay_work_repairs": 2}, max_repairs=2)
        is None
    )
...
def test_review_cannot_send_repairs_to_an_outside_or_future_work_item():
    ...
    for target in ("stranger", "final", "verify"):
        state = record_work_result(
            {},
            items[2],
            result("continue", findings=[{"workItemId": target, "note": "Fix"}]),
        )
...
def test_lead_plan_is_bounded_and_preserves_required_review():
    ...
    plan = [
        {
            "agentId": "builder",
            "objective": "Implement POST /reset",
            "acceptanceCriteria": ["Expired tokens rejected"],
            "expectedOutputs": ["API and tests"],
        },
        {
            "agentId": "reviewer",
            "objective": "Review reset token handling",
            "acceptanceCriteria": ["No reusable reset token"],
            "expectedOutputs": ["Findings with evidence"],
        },
    ]
            ...
            messages=[
                {
                    "kind": "question",
                    "toWorkItemId": "build",
                    "text": "Which input contract did you implement?",
                },
            ],
        ),
    )
            ...
            messages=[
                {
                    "kind": "answer",
                    "toWorkItemId": "verify",
                    "text": "Empty input is rejected with 400.",
                },
            ]
        ),
    )
...
def test_predecessor_context_is_bounded_and_marks_truncated_evidence():
    ...
    state = {
        WORK_RESULTS: {str(i): result(evidence=["x" * 2000] * 20) for i in range(18)}
    }
