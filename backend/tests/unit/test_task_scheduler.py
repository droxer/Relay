...
def test_scheduler_blocks_after_first_failed_dispatch() -> None:
    ...
    async def run_flow() -> None:
        with TemporaryDirectory() as root:
            ...
            assert second == SchedulerTickResult()
            assert backend.calls == 1
...
def test_scheduler_keeps_the_claim_on_ambiguous_dispatch_failure() -> None:
    ...
    claim is retained for reconciliation, but the task is blocked so lease
    expiry cannot trigger another automatic attempt."""
    ...
    async def run_flow() -> None:
        with TemporaryDirectory() as root:
            ...
            # A blocked task remains out of the automatic dispatch queue.
            assert second.dispatched == 0
