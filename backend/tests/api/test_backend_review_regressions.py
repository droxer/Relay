...
from relay.services.event_notifier import KeyedEventNotifier, database_notification_bridge
...
def test_static_assets_reject_sibling_prefix_and_symlink_escape(review_app, monkeypatch, tmp_path):
    dist = tmp_path / "out"
    dist.mkdir()
...
def test_artifact_download_never_reads_backend_host(review_app, review_client, tmp_path, content):
    workspace = tmp_path / "remote-workspace"
    workspace.mkdir()
    host_file = workspace / "report.txt"
    host_file.write_bytes(b"backend host private data")
    session = SessionController(
        review_app.state.session_store, workspace_path=str(workspace),
        owner_employee_id="admin",
    ).create_session("Remote report")
    artifact = {
        "id": new_database_id(), "kind": "workspace_file", "title": "report.txt",
        "path": str(host_file), "workspaceRelativePath": "report.txt",
        "createdAt": now_iso(), "contentType": "text/plain", "bytes": 15,
    }
    review_app.state.session_store.index_workspace_artifact(session["id"], artifact, content)
    response = review_client.get(f"/api/v1/threads/{session['id']}/artifacts/{artifact['id']}")
...
def test_artifact_snapshots_download_with_restricted_csp(review_app, review_client, content_type):
    session = SessionController(review_app.state.session_store, owner_employee_id="admin").create_session("Download")
    artifact = {
        "id": new_database_id(), "kind": "workspace_file", "title": 'report "你好".html',
        "path": "/remote/report.html", "workspaceRelativePath": "report.html",
        "createdAt": now_iso(), "contentType": content_type, "bytes": 25,
    }
...
def test_sandbox_provision_checks_existing_owner(review_app, review_client):
    for employee in ("alice", "bob"):
        assert review_client.post("/api/v1/admin/employees", json={
            "employeeId": employee, "username": employee, "password": "userpass",
        }).status_code == 201
    node = review_app.state.registry.register({
        "sandboxId": "review-alice", "employeeId": "alice", "token": "node_token",
        "protocolVersion": 1, "supportedAgents": ["codex"], "status": "ready",
    })
    assert review_client.post("/api/v1/auth/login", json={"username": "bob", "password": "userpass"}).status_code == 200
    response = review_client.post("/api/v1/sandboxes", json={"employeeId": "bob", "sandboxId": node["id"]})
    assert response.status_code == 403
    assert review_app.state.registry.get(node["id"])["employeeId"] == "alice"
    assert review_client.post("/api/v1/auth/login", json={"username": "alice", "password": "userpass"}).status_code == 200
    assert review_client.post("/api/v1/sandboxes", json={"employeeId": "alice", "sandboxId": node["id"]}).status_code == 201


def test_notification_bridge_preserves_password():
    engine = create_engine("postgresql+psycopg://review:dummy%40password@localhost/review")
    try:
        bridge = database_notification_bridge(SimpleNamespace(engine=engine), KeyedEventNotifier())
...
def test_terminal_thread_actions_update_linked_task(review_app, review_client, action, session_status, task_status):
    task = review_client.post("/api/v1/tasks", json={"title": "Linked task", "createSession": True}).json()
    ...
    response = review_client.post(f"/api/v1/threads/{session_id}/{endpoint}", json={"kind": action})
    assert response.status_code in (200, 202)
    assert response.json()["status"] == session_status
    assert review_app.state.task_store.get_task(task["id"])["status"] == task_status


def test_terminal_thread_action_rolls_back_on_task_write_failure(review_app, review_client, monkeypatch):
    task = review_client.post("/api/v1/tasks", json={"title": "Atomic completion", "createSession": True}).json()
    session_id = task["linkedSessionIds"][0]
    ...
    with pytest.raises(RuntimeError, match="injected task write failure"):
        review_client.post(f"/api/v1/threads/{session_id}/decisions", json={"kind": "mark_done"})
