...
def test_stop_and_delete_persists_intent_until_execution_exits(monkeypatch) -> None:
...
def test_execution_status_distinguishes_expired_lease_from_completed(monkeypatch) -> None:
    monkeypatch.setenv("RELAY_ADMIN_TOKEN", "admin_token")
    with TemporaryDirectory() as root:
        app = create_app(root)
        client = TestClient(app)
