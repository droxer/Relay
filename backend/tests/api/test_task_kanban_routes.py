...
def client(tmp_path, monkeypatch):
    ...
    response = client.post('/api/v1/auth/bootstrap', json={'token': 'test-kanban-token', 'username': 'admin', 'password': 'test-kanban-password'})
...
def test_new_tasks_require_review_and_cannot_fabricate_execution(client):
    ...
    for state in ['running', 'review', 'done']:
        assert client.post('/api/v1/tasks', json={'title': 'Invalid', 'status': state}).status_code == 400
...
def test_review_acceptance_and_block_restore(client):
    ...
    blocked = client.patch(url, json={'status': 'blocked', 'blockerReason': 'Need approval'}).json()
...
def test_acceptance_policy_validation(client):
    assert client.post('/api/v1/tasks', json={'title': 'Invalid', 'acceptancePolicy': 'anything'}).status_code == 400
    task = client.post('/api/v1/tasks', json={'title': 'Automated', 'acceptancePolicy': 'automatic'}).json()
