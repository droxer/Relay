...
def text_list(value: Any, name: str, *, limit: int = 20) -> list[str]:
    if not isinstance(value, list) or len(value) > limit or any(
        ...
        raise ValueError(f"{name} must contain at most {limit} nonempty strings (2000 characters each).")
...
def member_configs(value: Any, members: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for agent_id, config in value.items():
        if not isinstance(config, dict) or set(config) - {
            "role", "responsibility", "required", "participation", "expectedOutputs",
        }:
            raise ValueError("Unsupported membership configuration.")
        if "role" in config and config["role"] not in ROLES:
        ...
        if config.get("participation") == "on_request" and config.get("required", False):
        ...
        result[agent_id] = {
            **config,
            **({"responsibility": responsibility.strip()} if "responsibility" in config else {}),
            **({"expectedOutputs": text_list(config["expectedOutputs"], "expectedOutputs")} if "expectedOutputs" in config else {}),
        }
    return result
