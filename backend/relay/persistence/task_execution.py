...
def prepare_execution_events(
    ...
    if execution_owner is not None:
        ...
        if task.get("deletedAt") or (not legacy and owner != execution_owner):
            return []
    claims = [event for event in events if event.get("type") == "task.execution.claimed"]
    if not claims:
        return events
    if len(events) != 1:
    ...
    if (
        ...
        or task.get("deletedAt") or task.get("isRoutine")
    ):
...
def request_execution_owner(request: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "revision": (request.get("state") or {}).get("_relay_task_execution_revision", 0),
    }
