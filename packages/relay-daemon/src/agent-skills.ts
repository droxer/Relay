...
if(x.delivery.kind==='config-dir'){const cfg=inside(tmp,x.delivery.subdir);fs.mkdirSync(cfg,{recursive:true});const nodeCfg=inside(x.agentHome,x.delivery.subdir);if(fs.existsSync(nodeCfg)){for(const n of fs.readdirSync(nodeCfg)){if(n===x.delivery.skillsSubpath)continue;fs.symlinkSync(inside(nodeCfg,n),inside(cfg,n))}}const skills=inside(cfg,x.delivery.skillsSubpath);fs.mkdirSync(skills,{recursive:true});for(const s of x.skills){const to=inside(skills,...s.slug.split('/'));fs.mkdirSync(p.dirname(to),{recursive:true});fs.symlinkSync(inside(store,s.manifestSha256),to,'dir')}}else{const root=inside(tmp,'skills');fs.mkdirSync(root,{recursive:true});for(const s of x.skills){const to=inside(root,...s.slug.split('/'));fs.mkdirSync(p.dirname(to),{recursive:true});fs.symlinkSync(inside(store,s.manifestSha256),to,'dir')}}
fs.writeFileSync(inside(tmp,'.relay-view'),x.viewId,{mode:0o600});try{fs.renameSync(tmp,view)}catch(e){if(!fs.existsSync(view))throw e;fs.rmSync(tmp,{recursive:true,force:true});if(!regular(inside(view,'.relay-view'))||fs.readFileSync(inside(view,'.relay-view'),'utf8')!==x.viewId)throw Error('unsafe existing view');const root=x.delivery.kind==='config-dir'?inside(view,x.delivery.subdir,x.delivery.skillsSubpath):inside(view,'skills');const found=[];const walk=(d,b='')=>{for(const n of fs.readdirSync(d)){const q=inside(d,n),rel=b?b+'/'+n:n,st=fs.lstatSync(q);if(st.isSymbolicLink())found.push([rel,fs.readlinkSync(q)]);else if(st.isDirectory())walk(q,rel);else throw Error('unsafe existing view')}};walk(root);if(found.length!==x.skills.length||found.some(([slug,target])=>!x.skills.some(s=>s.slug===slug&&p.resolve(p.dirname(inside(root,...slug.split('/'))),target)===inside(store,s.manifestSha256))))throw Error('unsafe existing view')}
if(x.delivery.kind==='config-dir')env[x.delivery.envVar]=inside(view,x.delivery.subdir);else if(x.delivery.kind==='skills-dir-flag'){const root=inside(view,'skills');paths=x.skills.length?[...new Set(x.skills.map(s=>p.dirname(inside(root,...s.slug.split('/')))))]:[root]}else{const root=inside(view,'skills');paths=x.skills.map(s=>inside(root,...s.slug.split('/')))}process.stdout.write(JSON.stringify({skillPaths:paths,env}));`;
