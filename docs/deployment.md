...
3. **Connect a computer.** Open *Computers → Connect this computer*, register
   the workspace, and run the first-time setup command on the target machine.
   Run the daemon command in the same terminal and paste the node token at
   the hidden prompt. Confirm `--backend-url` is your public HTTPS API origin.
   Enrollment and token commands are generated by the backend: configure
   `RELAY_PUBLIC_BACKEND_URL` there, not just the web build's `NEXT_PUBLIC_*`
   settings. Keep the latter aligned for other client-generated copy-out values.

   The setup command downloads and builds the daemon from the Relay GitHub
   repository; it does not download an installer from your domain. Users need
   repository access, Git, Node.js 22.19+, and npm. The daemon command then
   connects outbound to your configured domain over HTTPS. Check that
   `https://<your-api-domain>/healthz` is reachable from the user's computer
   and that the computer becomes online in Relay after starting its daemon.
