...
export interface KanbanItemProps extends useRender.ComponentProps<"div"> {
  ...
  disabled?: boolean
  /**
   * Relay addition: the whole item is the drag handle. The item is also the
   * activator node, so Space/Enter on a control nested inside it (a checkbox,
   * a link) keeps its own meaning instead of picking the item up.
   */
  asHandle?: boolean
}
...
export interface KanbanColumnContentProps extends useRender.ComponentProps<"div"> {
  value: string
  /** Relay addition: how siblings make room during a drag. */
