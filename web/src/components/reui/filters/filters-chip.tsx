...
// Relay: ui/tooltip exports one composed <Tooltip content>; these call sites
// compose the parts, so they take base-ui's Root/Trigger directly.
...
/* Relay addition: which rule-menu rows a host offers. A host that persists
   filters in a flat store (Relay keeps one value per field in the URL) can
   represent neither a duplicated rule nor a negated one, so it turns those
   rows off rather than letting them build a filter it would silently drop. */
export const FilterRuleMenuOptionsContext = React.createContext<{
  duplicate: boolean
  negate: boolean
}>({ duplicate: true, negate: true })
...
export function FilterRuleMenuItems({
  ...
  const actions = useFilterActions()
  const ruleMenu = React.useContext(FilterRuleMenuOptionsContext)
