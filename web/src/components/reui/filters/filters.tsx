...
export function Filters<V = unknown, O = unknown>({
  ...
  React.useEffect(() => {
    ...
    if (draft.ruleId) {
      ...
      const id = nextId()
      // Relay addition: a field that offers exactly one condition has nothing
      // to choose at the condition step, so the rule starts on it and the
      // VALUE opens instead. With more than one, the condition stays empty on
      // purpose: the chip renders "Select condition" and opens.
      const field = getFilterField(latest.current.index, draft.path)
      const visible = field
        ? latest.current.resolveOperators(field).filter((operator) => !operator.hidden)
        : []
      ...
      focusStore.set({ id, segment: only ? "value" : "operator", autoOpen: true })
    }
    dispatchDraftRaw({ type: "close" })
