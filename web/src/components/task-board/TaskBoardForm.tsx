...
/* The task board's record form — every field of a task or a routine, its
   embedded sections (artifacts, workspace, run history), and its action row.
   No chrome of its own: it is mounted inside the backlog's drawer and inside
   the routine detail pane, which is the whole reason it lives apart from
   TaskDrawer. A second copy of these fields would be two forms to keep in
   step for one record. */

export type TaskBoardFormProps = {
  form: TaskBoardFormState;
  logicalAgents: EmployeeAgent[];
  teams?: AgentTeam[];
  saving: boolean;
  deleting?: boolean;
  /** Field that receives focus on mount. Quick-assign actions pass "assignment". */
  initialFocus?: "title" | "assignment";
  onChange: (next: TaskBoardFormState) => void;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
  onDelete?: () => void;
  /** Dismisses the surface around the form. The drawer passes its close; the
   *  detail pane omits it, because a pane has nothing to dismiss to. */
  onCancel?: () => void;
  /** Read-only context (status, linked thread, recent activity) shown above the fields. */
  meta?: ReactNode;
  ...
  return (
    <form className="adm-form task-board-drawer-form" onSubmit={handleSubmit} noValidate>
        ...
        <div className="adm-form-actions">
          ...
          {onCancel ? (
            <Button type="button" variant="ghost" size="cta" onClick={onCancel} disabled={busy}>
              {t("dialog.cancel")}
            </Button>
          ) : null}
